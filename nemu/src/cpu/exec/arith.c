...
make_EHelper(add) {
  rtl_add(&t2, &id_dest->val, &id_src->val);//t2 unsigned sum
  operand_write(id_dest, &t2);//ans = t2

  rtl_update_ZFSF(&t2, id_dest->width);//refresh ZFSF

  rtl_sltu(&t0, &t2, &id_dest->val);//t2<t3
  rtl_set_CF(&t0);//set CF

  rtl_xor(&t0, &id_dest->val, &id_src->val);//按位异或
  rtl_not(&t0);//取反
